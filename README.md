...
- **Encryption** uses `libsodium-wrappers` (pure WASM), which supports Discord's current voice modes.
...
3. Add the **Variables** from `.env.example` (at minimum `DISCORD_TOKEN` and `AZURACAST_STREAM_URL`). Set `GUILD_ID` to your server's ID so the commands show up instantly.
